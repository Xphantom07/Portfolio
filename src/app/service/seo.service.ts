...
export class SeoService {
  // Use your own site URL if you have one; otherwise keep as root for relative URLs
  private baseUrl = '';
  private siteName = 'Bhavik Vavadiya';
      ...
      .subscribe(route => {
        ...
        if (routeData) {
          ...
          this.updatePageMetadata({
            title: routeData['title'] || 'Bhavik Vavadiya | Data Analyst | Machine Learning Engineer | Portfolio',
            description: routeData['description'] || 'Data Analyst & Machine Learning Engineer Portfolio',
  ...
  private normalizeUrl(url: string): string {
    try {
      if (url.startsWith('/')) {
        ...
        const normalizedPath = rawPath !== '/' && rawPath.endsWith('/')
          ? rawPath.slice(0, -1)
          : rawPath;
        ...
        return queryString ? `${normalizedPath.toLowerCase()}?${queryString}` : normalizedPath.toLowerCase();
      }
  ...
  updateSeoForRoute(route: string, metadata: Partial<PageMetadata>): void {
    ...
    this.updatePageMetadata({
      title: metadata.title || 'Bhavik Vavadiya | Data Analyst | Machine Learning Engineer | Portfolio',
      description: metadata.description || 'Data Analyst & Machine Learning Engineer Portfolio',
      url: fullUrl,
      keywords: metadata.keywords,
